"""MCP Server — exposes NexusAI skills as MCP tools over HTTP."""
...
from nexusai.core.types import SkillContext, ToolDefinition
...
class MCPServer:
    """FastAPI-based HTTP server that exposes NexusAI skills as MCP tools.
    ...
    def _build_app(self) -> FastAPI:
        app = FastAPI(title="NexusAI MCP Server", version="1.0.0")
    ...
    def _skill_to_mcp_tool(self, tool_def: ToolDefinition) -> dict:
        """Convert a NexusAI ToolDefinition to an MCP tool descriptor dict."""
        return {
            "name": tool_def.name,
