...
class AutomationSkill:
    ...
    async def _check_environment(self, cwd: str) -> SkillResult:
        ...
        pkg_result = await self._run_command(
            ["python", "-c", "import nexusai; print(nexusai.__version__ if hasattr(nexusai, '__version__') else 'installed')"],
            cwd,
            timeout=10,
        )
        if pkg_result.success:
            pkg_ver = pkg_result.output.strip()
            lines.append(f"nexusai package: {pkg_ver} ✓")
        else:
            lines.append("nexusai package: not installed ✗")
            overall_ok = False
    ...
    async def _run_ci_pipeline(self, args: dict[str, Any], cwd: str) -> SkillResult:
        ...
        if not skip_lint:
            ok = await _step(
                "lint",
                ["python", "-m", "ruff", "check", "nexusai/", "--output-format=concise"],
                _LINT_TIMEOUT,
            )
            if not ok:
                return self._pipeline_result(steps, overall_ok, pipeline_start)
        ...
        if not skip_coverage:
            await _step(
                ...
                [
                    ...
                    "--cov=nexusai", "--cov-report=term-missing",
                    "-q", "--no-header",
                ],
                _RUN_TIMEOUT,
            )
    ...
    async def _watch_tests(self, args: dict[str, Any], cwd: str) -> SkillResult:
        watch_path = args.get("path", "nexusai/")
