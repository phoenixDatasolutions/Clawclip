"""High-level KnowledgeManager for the ClawClip RAG Knowledge Base module.

Provides CRUD operations on named knowledge bases and delegates the heavy
lifting to :class:`~clawclip.knowledge.indexer.DocumentIndexer` and
:class:`~clawclip.knowledge.retriever.Retriever`.
"""
...
class KnowledgeManager:
    ...
    ----------
    indexer:
        :class:`~clawclip.knowledge.indexer.DocumentIndexer` instance used
        to index new content.
    retriever:
        :class:`~clawclip.knowledge.retriever.Retriever` instance used to
    ...
    async def add_url(self, kb_id: str, url: str) -> int:
        ...
        """
        self._ensure_kb(kb_id)
        from clawclip.knowledge.loaders.web_loader import load_url
    ...
    async def add_git_repo(self, kb_id: str, repo_path: str) -> int:
        ...
        """
        self._ensure_kb(kb_id)
        from clawclip.knowledge.loaders.git_loader import load_repo
