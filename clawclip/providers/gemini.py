...
class GeminiProvider(BaseLLMProvider):
        ...
        if self._genai is None:
            try:
                ...
                raise ImportError(
                    ...
                    "Install with: pip install clawclip[gemini]"
                )
        return self._genai
    ...
    def _convert_messages(messages: list[LLMMessage]) -> list[Any]:
        """Convert ClawClip messages to Gemini Content format.
    ...
    def _convert_tools(tools: list[ToolDefinition]) -> Any:
        """Convert ClawClip tool definitions to Gemini function_declarations."""
        try:
            from google.generativeai import types as genai_types
        except ImportError:
            raise ImportError("google-generativeai package not installed.")
