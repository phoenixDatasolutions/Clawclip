...
class ClaudeAPIProvider(BaseLLMProvider):
    ...
    def _get_client(self) -> Any:
        ...
        if self._client is None:
            try:
                ...
                raise ImportError(
                    "anthropic package not installed. Install with: pip install clawclip[claude-api]"
                )
        return self._client
    ...
    def _convert_messages(messages: list[LLMMessage]) -> list[dict[str, Any]]:
        """Convert ClawClip messages to Anthropic API format."""
    ...
    def _convert_tools(tools: list[ToolDefinition]) -> list[dict[str, Any]]:
        """Convert ClawClip tool definitions to Anthropic format."""
        return [
            {
                "name": tool.name,
